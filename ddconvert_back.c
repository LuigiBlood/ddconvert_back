...
// Converts DD MAME image back to the original format
// Original converter by Happy_
// Modified converter by LuigiBlood
...
const unsigned int StartTrack [16] = {  0,158,316,465,614,763, 912,1061,
                                      158,316,465,614,763,912,1061,1175};
...
int main(int argc, char* argv[])
{
	...
	else
	{
		...
		{
			InOffset = OutStart[zone];
			OutOffset = InStart[RevDiskTypeZones[disktype][zone]];
		...
		{
			OutOffset = InStart[RevDiskTypeZones[disktype][zone]];
			InOffset = OutStart[RevDiskTypeZones[disktype][zone]];
			fseek(in, InOffset, SEEK_SET);
			fseek(out, OutOffset, SEEK_SET);
			block = StartBlock[disktype][zone];
			atrack = 0xB;
